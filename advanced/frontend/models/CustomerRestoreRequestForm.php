...
class CustomerRestoreRequestForm extends Model
{
    ...
    /**
     * Тихий сценарий восстановления доступа.
     *
     * Возвращает true и в случае успеха, и в случае отсутствующего / неактивного customer,
     * чтобы не раскрывать существование email в системе.
     */
    public function process(): bool
    {
        ...
        try {
            ...
            // Наружу всё равно не показываем, что что-то пошло не так
            return true;
        }
    ...
    protected function sendRestoreEmail(CustomerModel $customer, string $rawToken): bool
    {
        ...
        try {
            ...
            // Наружу опять же не палим ошибку
            return true;
        }
    }
}